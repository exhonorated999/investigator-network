/**
 * Live-training reminder cron trigger.
 *
 * Intended to run on a schedule (Railway Cron service, e.g. every 5 minutes:
 * cron expression `*​/5 * * * *`). It simply POSTs to the app's protected
 * reminder endpoint with the shared secret; all the logic lives server-side in
 * src/app/api/cron/live-reminders/route.ts.
 *
...
const endpoint = `${base}/api/cron/live-reminders`;
